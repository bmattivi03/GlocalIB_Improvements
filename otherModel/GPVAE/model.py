...
class GPVAE_MY(BaseNNImputer):
    ...
    def get_all_info(
        ...
        for idx, data in enumerate(data_loader):
            inputs = self._assemble_input_for_testing(data)
            results = self.model.get_all_info(inputs, n_sampling_times=n_sampling_times)
            imputed_data = results["imputed_data"]
            imputation_collector.append(imputed_data)
            enc_out_collector.append(results["enc_out"])

        imputation = torch.cat(imputation_collector).cpu().detach().numpy()
        enc_out = torch.cat(enc_out_collector).cpu().detach().numpy()
        result_dict = {
            "imputation": imputation,
            "enc_out": enc_out,
        }
