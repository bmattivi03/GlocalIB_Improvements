"""
Preprocessing func for the dataset traffic .
...
def load_traffic(local_path):
    """Load dataset traffic.

    Parameters
    ----------
    local_path : str,
        The local path of dir saving the raw data of traffic.

    Returns
    -------
    data : dict
        A dictionary contains the time-series data of traffic.
    """
    file_path = os.path.join(local_path, "traffic.csv")
...
def preprocess_traffic(
    ...
    **kwargs,
    ...
    """Load and preprocess the dataset traffic.
    ...
    -------
    processed_dataset :
        A dictionary containing the processed traffic.
    """
    ...
    df = load_traffic("/root/work/myModel/data/all_datasets/traffic")
    ...
    selected_as_train = unique_months[:15]  # use the first 23 months as train set
    logger.info(f"months selected as train set are {selected_as_train}")
    selected_as_val = unique_months[15:20]  # select the following 7 months as val set
    logger.info(f"months selected as val set are {selected_as_val}")
    selected_as_test = unique_months[20:]  # select the left 7 months as test set
