"""
Preprocessing func for the dataset weather .
...
def load_weather(local_path):
    """Load dataset weather.

    Parameters
    ----------
    local_path : str,
        The local path of dir saving the raw data of weather.

    Returns
    -------
    data : dict
        A dictionary contains the time-series data of weather.
    """
    file_path = os.path.join(local_path, "weather.csv")
...
def preprocess_weather(
    ...
    **kwargs,
    ...
    """Load and preprocess the dataset weather.
    ...
    -------
    processed_dataset :
        A dictionary containing the processed weather.
    """
    ...
    df = load_weather("/root/work/myModel/data/all_datasets/weather")
    ...
    selected_as_train = unique_months[:7]  # use the first 23 months as train set
    logger.info(f"months selected as train set are {selected_as_train}")
    selected_as_val = unique_months[7:10]  # select the following 7 months as val set
    logger.info(f"months selected as val set are {selected_as_val}")
    selected_as_test = unique_months[10:]  # select the left 7 months as test set
