"""
Preprocessing func for the dataset illness .
...
def load_illness(local_path):
    """Load dataset illness.

    Parameters
    ----------
    local_path : str,
        The local path of dir saving the raw data of illness.

    Returns
    -------
    data : dict
        A dictionary contains the time-series data of illness.
    """
    file_path = os.path.join(local_path, "national_illness.csv")
    df = muu.read_csv_tqdm(file_path, index_col="date")
    df.index = pd.to_datetime(df.index)
    return df


def preprocess_illness(
    rate,
    n_steps,
    pattern: str = "point",
    **kwargs,
) -> dict:
    """Load and preprocess the dataset illness.
    ...
    -------
    processed_dataset :
        A dictionary containing the processed illness.
    """
    ...
    df = load_illness("/root/work/myModel/data/all_datasets/illness")
    ...
    selected_as_train = unique_months[:134]  # use the first 23 months as train set
    logger.info(f"months selected as train set are {selected_as_train}")
    selected_as_val = unique_months[134:178]  # select the following 7 months as val set
    logger.info(f"months selected as val set are {selected_as_val}")
    selected_as_test = unique_months[178:]  # select the left 7 months as test set
