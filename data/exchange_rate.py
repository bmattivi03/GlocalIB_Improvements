"""
Preprocessing func for the dataset exchange_rate .
...
def load_exchange_rate(local_path):
    """Load dataset exchange_rate.

    Parameters
    ----------
    local_path : str,
        The local path of dir saving the raw data of exchange_rate.

    Returns
    -------
    data : dict
        A dictionary contains the time-series data of exchange_rate.
    """
    file_path = os.path.join(local_path, "exchange_rate.csv")
    df = muu.read_csv_tqdm(file_path, index_col="date")
    df.index = pd.to_datetime(df.index)
    return df


def preprocess_exchange_rate(
    rate,
    n_steps,
    pattern: str = "point",
    **kwargs,
) -> dict:
    """Load and preprocess the dataset exchange_rate.
    ...
    -------
    processed_dataset :
        A dictionary containing the processed exchange_rate.
    """
    ...
    df = load_exchange_rate("/root/work/myModel/data/all_datasets/exchange_rate")
    ...
    selected_as_train = unique_months[:150]  # use the first 23 months as train set
    logger.info(f"months selected as train set are {selected_as_train}")
    selected_as_val = unique_months[150:200]  # select the following 7 months as val set
    logger.info(f"months selected as val set are {selected_as_val}")
    selected_as_test = unique_months[200:]  # select the left 7 months as test set
