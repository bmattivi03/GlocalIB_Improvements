"""
Preprocessing func for the dataset Electricity .
...
def load_electricity(local_path):
    ...
    ----------
    local_path : str,
        The local path of dir saving the raw data of Electricity.

    Returns
    -------
    data : dict
        A dictionary contains the time-series data of Electricity.
    """
    file_path = os.path.join(local_path, "electricity.csv")
...
def preprocess_electricity(
    ...
    **kwargs,
    ...
    """Load and preprocess the dataset Electricity.
    ...
    -------
    processed_dataset :
        A dictionary containing the processed Electricity.
    """
    ...
    df = load_electricity("/root/work/myModel/data/all_datasets/electricity")
    ...
    selected_as_train = unique_months[:23]  # use the first 23 months as train set
    logger.info(f"months selected as train set are {selected_as_train}")
    selected_as_val = unique_months[23:30]  # select the following 7 months as val set
    logger.info(f"months selected as val set are {selected_as_val}")
    selected_as_test = unique_months[30:]  # select the left 7 months as test set
